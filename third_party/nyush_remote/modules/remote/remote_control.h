...
#define KB_ZE_DISABLE

typedef enum
{
    REMOTE_CONTROL_PROTOCOL_UNKNOWN = 0,
    REMOTE_CONTROL_PROTOCOL_DBUS = 1,
    REMOTE_CONTROL_PROTOCOL_VIDEO_TRANSMITTER = 2,
} Remote_Control_Protocol_e;
...
typedef struct
{
    ...
    struct
    {
        uint8_t mode_switch;   // VT03/VT13 三挡开关原始值: C=0, N=1, S=2
        uint8_t pause;         // VT03/VT13 暂停键
        uint8_t custom_left;   // VT03/VT13 左侧自定义键
        uint8_t custom_right;  // VT03/VT13 右侧自定义键
        uint8_t trigger;       // VT03/VT13 扳机键
        uint8_t pause_count;   // 暂停键上升沿累计计数,用于切换视觉模式
    } vtm;
 ...
 *            - DBUS 遥控数据使用 USART3 (100000 9E1)
 *            - VT03/VT13 图传遥控数据使用 USART1 (921600 8N1)
 *
 */
 ...
 * @brief 获取当前遥控器数据快照
 *
 * @return const RC_ctrl_t* 未初始化时返回NULL
 */
 ...
 * @brief 获取当前遥控协议类型
 *
 * @return Remote_Control_Protocol_e 未初始化时返回 UNKNOWN
 */
